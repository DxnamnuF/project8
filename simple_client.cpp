...
// Фиксированный ключ, который клиент отправляет в handshake – "hello, world"
...
// MinimalPerformHandshakeClient – клиентский handshake.
// Формирует HTTP‑запрос для WebSocket, используя фиксированный Sec-WebSocket-Key ("hello, world"),
// и проверяет, что ответ содержит "101 Switching Protocols" и Accept-ключ "hello, kitty".
bool MinimalPerformHandshakeClient(int sockfd, const string &host, int port, const string &client_key) {
    ...
    // Проверяем, что сервер прислал фиксированный Accept-ключ "hello, kitty"
...
int main(int argc, char *argv[]){
    ...
    // После успешного handshake завершаем работу.
    cout << "Handshake verified. Closing connection." << endl;
    return 0;
}
